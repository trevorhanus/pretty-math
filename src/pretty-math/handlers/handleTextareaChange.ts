...
import { BlockRange } from 'pretty-math/selection/BlockRange';
...
export function handleTextareaChange(editorState: Editor, e: React.ChangeEvent<HTMLTextAreaElement>) {
    ...
    // Fraction
    if (keyValue === '/') {
        insertFraction(editorState);
        return;
    }

    if (keyValue === '^') {
        insertSuperscript(editorState);
        return;
    }
...
export function insertFraction(editor: Editor) {
...
export function insertSuperscript(editorState: Editor) {
    const { focus } = editorState.selection;
    const { prev } = focus;
    if (prev && prev.type === 'math:supsub') {
